...
ScenarioResult run_scenario(int efficiency_factor, int gluttony) {
    ...
    bool cook_fired = false;      // Повар уволен
    bool cook_resigned = false;   // Повар уволился
    bool all_fatties_dead = false; // Все толстяки умерли
    int dead_count = 0;           // Счетчик умерших толстяков
    ...
    thread cook([&] {
        while (true) {
            unique_lock<CustomMutex> lk(mtx);
